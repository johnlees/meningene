...
my @cbpA_vals = (0, 1, 2, 3, 4, 5, 6);
my @pspC_vals = (0, 7, 8, 9, 10, 11);
...
my @colnames;
foreach my $val (@cbpA_vals)
{
   push(@colnames, "CBPA_$val");
}
foreach my $val (@pspC_vals)
{
   push(@colnames, "PSPC_$val");
}
...
while (my $line_in = <PSPC>)
{
   ...
   my ($lane, $cbpA_allele, $pspC_allele) = split(/\s+/, $line_in);
